...
class FieldMatrix
{
  ...
  */
  ...
  /**
	   Computes and returns the inverse of the matrix. The matrix must be invertible.
   */
  FieldMatrix inverse()const;
  /**
  ...
  */
  ...
  void appendRow(FieldVector const &r){assert(width==r.size());rows.push_back(r);height++;}
};
...
/**
 * Computes a basis of the quotient lattice of two saturated lattices.
 * The lattice kernel of "equations" is a lattice A which contains the sublattice B which is the
 * lattice kernel of "euqations union additionalEquations".
 * The routine returns a minimal set of vectors which will extend a lattice basis of B to a lattice basis of A.
 */
IntegerVectorList basisOfQuotientLattice(IntegerVectorList const &equations, IntegerVectorList const &additionalEquations, int n);
